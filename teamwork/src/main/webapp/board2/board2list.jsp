					...
					<c:if test="${l1.reply_count2 ne 0}">
                      ...
                      <td><c:choose>
							<c:when test="${not empty l1.modifyDate2 }">
								<fmt:formatDate value="${l1.modifyDate2 }" pattern="yyyy-MM-dd"/>
							</c:when>
							<c:otherwise>
								<fmt:formatDate value="${l1.createDate2 }" pattern="yyyy-MM-dd"/>
							</c:otherwise>
						</c:choose></td>
                      <td>${l1.hit2 }</td>
                  </tr>
					...
					<c:if test="${l2.reply_count2 ne 0}">
                      ...
                      <td><c:choose>
							<c:when test="${not empty l2.modifyDate2 }">
								<fmt:formatDate value="${l2.modifyDate2 }" pattern="yyyy-MM-dd"/>
							</c:when>
							<c:otherwise>
								<fmt:formatDate value="${l2.createDate2 }" pattern="yyyy-MM-dd"/>
							</c:otherwise>
						</c:choose></td>
                      <td>${l2.hit2 }</td>
                  </tr>
					...
					<c:if test="${l3.reply_count2 ne 0}">
                      ...
                      <td><c:choose>
							<c:when test="${not empty l3.modifyDate2 }">
								<fmt:formatDate value="${l3.modifyDate2 }" pattern="yyyy-MM-dd"/>
							</c:when>
							<c:otherwise>
								<fmt:formatDate value="${l3.createDate2 }" pattern="yyyy-MM-dd"/>
							</c:otherwise>
						</c:choose></td>
                      <td>${l3.hit2 }</td>
                  </tr>
					...
					<c:if test="${b.reply_count2 ne 0}">
                      ...
                      <td><c:choose>
							<c:when test="${not empty b.modifyDate2 }">
								<fmt:formatDate value="${b.modifyDate2 }" pattern="yyyy-MM-dd"/>
							</c:when>
							<c:otherwise>
								<fmt:formatDate value="${b.createDate2 }" pattern="yyyy-MM-dd"/>
							</c:otherwise>
						</c:choose></td>
                      <td>${b.hit2 }</td>
                  </tr>
     ...
     <!-- 페이지 처리 영역 -->
     	<section id="bottomlist">
				<div class = "pagination">
					<a href="/board2list.do?pageNum=1"><i class="fa-solid fa-forward fa-rotate-180"></i></a>
					<c:choose>
					<c:when test="${startPage > 1 }">
					<a href="/board2list.do?pageNum=${startPage-1}"><i class="fa-solid fa-play fa-rotate-180"></i></a>
					</c:when>
					<c:otherwise>
					...
					<c:forEach var = "i" begin = "1" end = "${endPage}">
						<c:if test="${page == i}">
							<a href="/board2list.do?pageNum=${i}"><b>${i}</b></a>
						</c:if>
						<c:if test="${page != i}">
							<a href="/board2list.do?pageNum=${i}">${i}</a>
						</c:if>
					</c:forEach>
					<!-- 다음페이지 -->
					<c:choose>
					<c:when test="${endPage > startPage }">
					<a href="/board2list.do?pageNum=${startPage+1}"><i class="fa-solid fa-play"></i></a>
					</c:when>
					<c:otherwise>
					<a href=""><i class="fa-solid fa-play"></i></a>
					</c:otherwise>
					</c:choose>
					<a href="/board2list.do?pageNum=${endPage}"><i class="fa-solid fa-forward"></i></a>	
				</div>
	<!-- 검색 -->
	<div class="search-container">
	    ...
	    <div class="input-wrapper">
	        <select name="field" id="search-select">
	            <option value="title2" ${(field eq "title") ? "selected" : "" }>제목</option>
